        ...
        <div style={{
          ...
          <div style={{ background: '#fff', padding: 24, borderRadius: 8, minWidth: 320, maxWidth: 400 }}
            ...
            {editingTask ? (
              <>
                <h2>Edit Task</h2>
                <input value={editFields.title} onChange={e => setEditFields({...editFields, title: e.target.value})} style={{ width: '100%', marginBottom: 8, padding: 6 }} />
                <input value={editFields.assignee} onChange={e => setEditFields({...editFields, assignee: e.target.value})} style={{ width: '100%', marginBottom: 8, padding: 6 }} />
                <select value={editFields.status} onChange={e => setEditFields({...editFields, status: e.target.value})} style={{ width: '100%', marginBottom: 8, padding: 6 }}>
                  <option value="todo">To-Do</option>
                  <option value="doing">Doing</option>
                  <option value="done">Done</option>
                </select>
                <select value={editFields.priority} onChange={e => setEditFields({...editFields, priority: e.target.value})} style={{ width: '100%', marginBottom: 8, padding: 6 }}>
                  <option value="high">High</option>
                  <option value="medium">Medium</option>
                  <option value="low">Low</option>
                </select>
                <input type="date" value={editFields.dueDate} onChange={e => setEditFields({...editFields, dueDate: e.target.value})} style={{ width: '100%', marginBottom: 8, padding: 6 }} />
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  <button onClick={saveEdit} style={{ background: '#1e90ff', color: '#fff', padding: '6px 12px', borderRadius: 4 }}>Save</button>
                  <button onClick={() => setEditingTask(null)} style={{ padding: '6px 12px', borderRadius: 4 }}>Cancel</button>
                </div>
              </>
            ) : (
              <>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                  <button onClick={() => startEdit(selectedTask)} style={{ padding: '6px 12px', background: '#1e90ff', color: '#fff', borderRadius: 4 }}>Edit</button>
                  <button onClick={() => deleteTask(selectedTask)} style={{ padding: '6px 12px', background: '#d93025', color: '#fff', borderRadius: 4 }}>Delete</button>
                  <button onClick={() => setSelectedTask(null)} style={{ padding: '6px 12px', borderRadius: 4 }}>Close</button>
                </div>
              </>
            )}
          </div>
